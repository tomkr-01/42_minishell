...
static void	print_error_and_exit(char *command, char *message, int exit_code)
{
	put_stderr(SHELL, command, NULL, message);
...
static int	command_check(char *command, char *path)
{
	struct stat		statbuf;
